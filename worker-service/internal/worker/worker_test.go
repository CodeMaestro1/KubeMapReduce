...
func TestWorker_MapUsesPerSplitMetadata(t *testing.T) {
	store := newMockStorage()
	...
	case data := <-inputSeen:
		got := string(data)
		if !strings.Contains(got, `"key":"apple"`) || !strings.Contains(got, `"key":"banana"`) {
			t.Fatalf("mapper input did not include both splits: %q", got)
		}
	case <-time.After(2 * time.Second):
		t.Fatal("mapper input was not captured")
	}
}

func TestWorker_MapRejectsBadPerSplitChecksum(t *testing.T) {
	store := newMockStorage()
	part1 := []byte(`{"key":"apple","value":"1"}` + "\n")
	part2 := []byte(`{"key":"banana","value":"2"}` + "\n")
	store.put("mapreduce-inputs", "part-1.jsonl", part1)
...
func TestWorker_SIGTERMCausesTaskFailed(t *testing.T) {
	t.Skip("SIGTERM lifecycle testing requires TaskStream mock integration")
	store := newMockStorage()
...
func TestWorker_SIGTERMReportsFailureBeforeExecReturns(t *testing.T) {
	...
	case <-failureStarted:
	case <-time.After(2 * time.Second):
		t.Fatal("TaskFailed was not started after SIGTERM")
	}
	...
	if failedReq == nil || failedReq.TaskId != "task-1" {
		t.Fatalf("unexpected failedReq: %+v", failedReq)
	}
}
