...
func TestScheduler_FailStaleTasks_DeduplicatesJobCancellation(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(QuerySelectStaleTasks)).
		WillReturnRows(sqlmock.NewRows([]string{"task_id", "attempt_id", "job_id", "attempt_count"}).
			AddRow(task1, attempt1, jobID, 3).
			AddRow(task2, attempt2, jobID, 3))
...
func TestScheduler_GetSystemConfig(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("failed to open mock db: %v", err)
	}
	...
	mock.ExpectQuery(regexp.QuoteMeta(QueryGetSystemConfig)).
		WillReturnRows(sqlmock.NewRows([]string{"max_concurrent_pods", "cpu_limit", "memory_limit", "worker_replicas", "max_jobs_per_node"}).
			AddRow(10, "1", "1Gi", 3, 5))

	s := &Scheduler{db: db}

	config, err := s.GetSystemConfig(context.Background())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	if config.MaxConcurrentPods != 10 || config.CPULimit != "1" || config.MemoryLimit != "1Gi" || config.WorkerReplicas != 3 || config.MaxJobsPerNode != 5 {
		t.Errorf("unexpected config: %+v", config)
	}
}

func TestScheduler_DetermineCleaningTerminalState(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("failed to create mock db: %v", err)
	}
	...
	mock.ExpectQuery("SELECT COUNT\\(\\*\\) FROM TASKS WHERE job_id = \\$1 AND status = 'Failed'").
		WithArgs("job-1").
		WillReturnRows(sqlmock.NewRows([]string{"count"}).AddRow(1))

	state, err := s.determineCleaningTerminalState(context.Background(), "job-1")
	if err != nil || state != "Failed" {
		t.Fatalf("expected Failed, got %s, %v", state, err)
	}

	mock.ExpectQuery("SELECT COUNT\\(\\*\\) FROM TASKS WHERE job_id = \\$1 AND status = 'Failed'").
		WithArgs("job-2").
		WillReturnRows(sqlmock.NewRows([]string{"count"}).AddRow(0))
	mock.ExpectQuery("SELECT COUNT\\(\\*\\) FROM TASKS WHERE job_id = \\$1 AND status != 'Completed'").
		WithArgs("job-2").
		WillReturnRows(sqlmock.NewRows([]string{"count"}).AddRow(1))

	state, err = s.determineCleaningTerminalState(context.Background(), "job-2")
	if err != nil || state != "Failed" {
		t.Fatalf("expected Failed, got %s, %v", state, err)
	}
}
