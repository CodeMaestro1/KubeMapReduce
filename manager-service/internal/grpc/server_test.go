...
func TestFindMapSplitForTask_NoSplits(t *testing.T) {
	task := &manager.Task{
		InputSplits: []manager.TaskInputSplit{},
	}

	split, ok := findMapSplitForTask(task)
	if ok {
		t.Fatalf("expected no split, got %v", split)
	}
}

func TestRuntimeEnvFromCodeURI(t *testing.T) {
	tests := []struct {
		uri      string
		expected string
	}{
	...
	for _, tt := range tests {
		got := runtimeEnvFromCodeURI(tt.uri)
		if got != tt.expected {
			t.Errorf("runtimeEnvFromCodeURI(%q) = %q; want %q", tt.uri, got, tt.expected)
		}
	}
}

func TestPopulateTaskTypeSpecifics_NoSplits(t *testing.T) {
	task := &manager.Task{
		Type:        manager.MapTask,
		InputSplits: []manager.TaskInputSplit{},
	}
	assignment := &pb.TaskAssignment{}
	populateTaskTypeSpecifics(task, assignment)

	if assignment.Type != pb.TaskType_MAP {
		t.Fatalf("expected Type %v, got %v", pb.TaskType_MAP, assignment.Type)
	}
}

func TestPopulateTaskTypeSpecifics_ReduceWithChecksum(t *testing.T) {
	task := &manager.Task{
		Type: manager.ReduceTask,
		ShuffleInputs: []manager.TaskOutputRef{
			{OutputURI: "s3://bucket/output", Checksum: "sha256-checksum"},
		},
	}
	assignment := &pb.TaskAssignment{}
	populateTaskTypeSpecifics(task, assignment)

	if assignment.Type != pb.TaskType_REDUCE {
		t.Fatalf("expected Type %v, got %v", pb.TaskType_REDUCE, assignment.Type)
	}
	if len(assignment.DataLocations) != 1 {
		t.Fatalf("expected 1 data location, got %d", len(assignment.DataLocations))
	}
	if assignment.DataLocations[0] != "s3://bucket/output#sha256=sha256-checksum" {
		t.Fatalf("unexpected data location: %s", assignment.DataLocations[0])
	}
}
